...
validationSetMSE_(0), generalizationSetMSE_(0) {
 ...
 ********************************************************************/
void
neuralNetworkTrainer::runTrainingEpoch(const vector<dataEntry *> &trainingSet) {
    double incorrectPatterns(0), mse(0);

    //for every training pattern
    for (int tp = 0; tp < (int) trainingSet.size(); tp++) {
        //feed inputs through network and backpropagate errors
        NN_->feedForward(trainingSet[tp]->patternV());
        backpropagate(trainingSet[tp]->targetV());
        ...
        for (int k = 0; k < NN_->nOutput_; k++) {
            ...
            if (NN_->clampOutput(NN_->outputNeurons_[k]) !=
                    trainingSet[tp]->targetV()[k])
                patternCorrect = false;

            //calculate MSE
            mse += pow((NN_->outputNeurons_[k] - trainingSet[tp]->targetV()[k]), 2);
        }
 ...
 ********************************************************************/
void
neuralNetworkTrainer::backpropagate(const vector<double> &desiredOutputs) {
