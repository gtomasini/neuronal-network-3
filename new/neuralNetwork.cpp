 ...
 ********************************************************************/
int*
neuralNetwork::feedForwardPattern(const vector<double> &pattern) {
    feedForward(pattern);
 ...
 *******************************************************************/
double
neuralNetwork::getSetAccuracy(const vector<dataEntry *>& set) {
    double incorrectResults = 0;

    //for every training input array
    for (int tp = 0; tp < (int) set.size(); tp++) {
        //feed inputs through network and backpropagate errors
        feedForward( set[tp]->patternV() );
        ...
        for (int k = 0; k < nOutput_; k++)
            ...
            if (clampOutput(outputNeurons_[k]) != set[tp]->targetV()[k]) {
                correctResult = false;
                break;
            }
 ...
 ********************************************************************/
double
neuralNetwork::getSetMSE(const vector<dataEntry *>& set) {
    double mse = 0;
    ...
    for (int tp = 0; tp < (int) set.size(); tp++) {
        ...
        for (int k = 0; k < nOutput_; k++)
            ...
            mse += pow((outputNeurons_[k] - set[tp]->targetV()[k]), 2);
    }//end for
 ...
 ********************************************************************/
void
neuralNetwork::feedForward(const vector<double> &pattern) {
