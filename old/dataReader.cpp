...
dataReader::loadDataFile( const char* filename, int nI, int nT ){
	...
	if ( inputFile.is_open() ){
		...
		//generalization set
		for ( int i = trainingDataEndIndex_; i < trainingDataEndIndex_ + gSize; i++ ) tSet_.generalizationSet_.push_back( data_[i] );
				
		//validation set
		for ( int i = trainingDataEndIndex_ + gSize; i < (int) data_.size(); i++ ) tSet_.validationSet_.push_back( data_[i] );
...
void dataReader::processLine( string &line ){
	...
	while ( t!=NULL && i < (nInputs_ + nTargets_) ){
		float auxf=atof(t);
